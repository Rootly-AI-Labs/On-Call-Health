...
def test_manual_delivery_succeeds_even_if_notification_creation_fails():
    current_user = SimpleNamespace(
        id=1,
        email="admin@example.com",
        role="admin",
    ...
    db = _build_db(SimpleNamespace(id=7), SimpleNamespace(message_template=None))
    ...
    with patch(
        ...
        "app.services.slack_dm_sender.SlackDMSender.send_survey_dm",
        return_value=True,
    ), patch(
        "app.services.notification_service.NotificationService.create_survey_delivery_notification",
